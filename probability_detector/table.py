...
class Table:
    ...
    def view_result(self):
        player_rank = np.zeros(self.num_players, dtype=int)
        player_hand_type = np.zeros(self.num_players, dtype=int)
...
class HoldemTable(Table):
    ...
    def simulate_calculation(self, community_cards, undrawn_combos):
        res_arr = np.zeros(shape=(len(undrawn_combos), self.num_players), dtype=int)
